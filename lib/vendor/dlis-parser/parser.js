...
function scanRecords(bytes, warnings) {
  ...
  while (pos + 4 <= fileSize) {
    ...
    if (vrMark !== 0xFF) throw new Error('Cabeçalho de registro inválido: leitura interrompida sem recuperação heurística.');

    if (vrLen < 4 || pos + vrLen > fileSize) throw new Error('Registro visível truncado.');
    ...
    while (lrsPos + 4 <= vrEnd) {
      ...
      if (lrsAttr & 0x10) throw new Error('Arquivo contém registro criptografado não suportado.');
      ...
      if (hasPad && bodyEnd > bodyStart) {
        ...
        if (nPad < 1 || nPad > bodyEnd-bodyStart) throw new Error('Padding RP66 inválido.');
        bodyEnd -= nPad;
      }
      ...
      if (isLast && curLR) {
        ...
        if (totalLen > 0) {
          ...
          if (curLR.isEFLR) {
            if (curLR.lrsType === 0) logicalIndex++;
            eflrRecords.push({ data: assembled, lrsType: curLR.lrsType, logicalIndex: Math.max(0,logicalIndex) });
            ...
            try {
              ...
              const key        = Math.max(0,logicalIndex) + '|' + obnameKey(frameRef);
...
function processOrigin(eflr, out) {
  for (const obj of eflr.objects) {
    out.origins.push({
      ...
      runNumber:    getAttr(obj, 'RUN-NUMBER') ?? null,
...
function processChannels(eflr, channels) {
  for (const obj of eflr.objects) {
    ...
    const repcode   = rc_raw  != null ? normalizeRC(rc_raw)  : (prev?.repcode ?? null);
    const dimension = dim     != null ? dim                   : (prev?.dimension ?? []);
    const props = (() => {
...
function decodeFrame(frameMeta, channels, iflrBlocks) {
  ...
  for (const ref of frameMeta.channelRefs) {
    ...
    if (!ch) throw new Error('Canal referenciado sem metadados exatos: ' + key);
    if (!ch.dimension.length || ch.dimension.some(n=>!Number.isInteger(n)||n<1) || ![1,2,5,6,7,10,11,12,13,14,15,16,24].includes(ch.repcode)) throw new Error('Dimensão ou representação não suportada: ' + ch.name);
    if (chanList.some(c=>c.name===ch.name)) throw new Error('Mnemonic duplicado no mesmo frame: ' + ch.name);
    chanList.push({...ch});
  }
  ...
  for (const blk of iflrBlocks) {
    ...
    while (!r.eof()) {
      ...
      if (!ok) throw new Error('Frame truncado ou representação não suportada. Nenhum resultado parcial será usado.');
...
export class DLISFile {
  ...
  static fromBuffer(buffer) {
    ...
    // STRATA strict fork: keep each FILE-HEADER scope separate. Never merge
    // channel/frame identities across logical files and never reconcile by name.
    const logicalFiles=[];
    const groupIds=[...new Set(eflrRecords.map(r=>r.logicalIndex))];
    for(const group of groupIds){
      const channels=new Map(), frames=new Map(), parameters=[], origins=[], fileHeaders=[];
      for(const rec of eflrRecords.filter(r=>r.logicalIndex===group)){
        try{
          const eflr=parseEFLR(rec.data), st=eflr.type.toUpperCase();
          switch(st){
            case 'FILE-HEADER': processFileHeader(eflr,{fileHeaders});break;
            case 'ORIGIN': processOrigin(eflr,{origins});break;
            case 'CHANNEL': processChannels(eflr,channels);break;
            case 'FRAME': processFrames(eflr,rec.data,frames);break;
            case 'PARAMETER': processParameters(eflr,parameters);break;
          }
        }catch(e){warnings.push('Logical file '+group+': metadados não decodificados: '+e.message);}
      }
      const localMap=new Map();
      for(const [key,blocks] of iflrMap) if(key.startsWith(group+'|')) localMap.set(key.slice(String(group).length+1),blocks);
      const origin=origins[0]||null;
      logicalFiles.push(new LogicalFile({id:fileHeaders[0]?.id||'',origin,channels,frames,parameters,iflrMap:localMap}));
    }
